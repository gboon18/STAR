void vz_corr(){
  ...
  for(int i_vz = 0 ; i_vz < 30 ; i_vz++){
    par_erf_corr_zr[i_vz] = par_erf_zr[17]/par_erf_zr[i_vz];
    par_erf_corr_ru[i_vz] = par_erf_ru[17]/par_erf_ru[i_vz];
    cout<<"\\\\ text{["<<vz[i_vz]-1<<", "<<vz[i_vz]+1<<")}"<<" & "<<par_erf_zr[i_vz]<<" & "<<par_erf_corr_zr[i_vz]<<" & "<<par_erf_ru[i_vz]<<" & "<<par_erf_corr_ru[i_vz]<<"\\\\"<<endl;
  }
  ...
  TFile *f_out = new TFile("output/erf.root" , "recreate");
  f_out->cd();
